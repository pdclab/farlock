...
#include "third_party/locks/rdma_spin_lock.h"
namespace lock {
class RdmaSpinHandler {
   public:
    RdmaSpinHandler(std::shared_ptr<rdma::MemoryPool> pool,
        ...
        lock_ = std::make_unique<RdmaSpinLockHandle>(
            node_id, cap, local_clients, config.local_budget_,
            config.remote_budget_);
        lock_->Init();
    }
    void Lock(const rdma::Pointer& lock) {
        lock_->Lock(rdma_ptr<RdmaSpinLock>(lock.raw()));
    }
    void Unlock(const rdma::Pointer& lock) {
        lock_->Unlock(rdma_ptr<RdmaSpinLock>(lock.raw()));
    }
    ~RdmaSpinHandler() {}

   protected:
    const std::shared_ptr<rdma::MemoryPool> pool_;
    std::unique_ptr<RdmaSpinLockHandle> lock_;
};
};  // namespace lock
