...
#include "third_party/locks/rdma_mcs_lock.h"
namespace lock {
class RdmaMcsHandler {
   public:
    RdmaMcsHandler(std::shared_ptr<rdma::MemoryPool> pool,
        ...
        lock_ = std::make_unique<RdmaMcsLockHandle>(node_id, cap, local_clients,
                                                    config.local_budget_,
                                                    config.remote_budget_);
        lock_->Init();
    }
    void Lock(const rdma::Pointer& lock) {
        lock_->Lock(rdma_ptr<RdmaMcsLock>(lock.raw()));
    }
    void Unlock(const rdma::Pointer& lock) {
        lock_->Unlock(rdma_ptr<RdmaMcsLock>(lock.raw()));
    }
    ~RdmaMcsHandler() {}

   protected:
    const std::shared_ptr<rdma::MemoryPool> pool_;
    std::unique_ptr<RdmaMcsLockHandle> lock_;
};
};  // namespace lock
